...
<head>
    ...
    <title>All AfterCare List</title>
   <!-- BOOTSTRAP STYLES-->
                ...
                <ul class="nav" id="main-menu">
                    ...
                    <li class="active-link">
                        <a href="<c:url value='/mine/mypage'>
					      </c:url>"><i class="fa fa-user "></i>마이 페이지<hr><i class="fa fa-thumbs-up " aria-hidden="true"></i>사후 관리 게시판 </a>
                    </li>
                    <li> 
                ...
                <hr />
                    ...
                    <div class="col-lg-6 col-md-6">                    	
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                            	<% int i=1; %>
                            	<c:forEach var="after" items="${aftercare_list}">
                            	<tr>
                            		<td><%out.print(i); i++; %></td>
                                    <td><a href="<c:url value='/aftercare/content'>
					  				 <c:param name='ac_list_id' value='${after.ac_list_id}'/>
					  				 <c:param name='owner_id' value='${owner_id}'/>
					  				 <c:param name='app_id' value='${app_id}'/>
			 		 				</c:url>">
			  						${after.ac_list_title}</a>
                                    </td>
                                    <td>${after.writer}</td>
                                </tr>
                            	</c:forEach>
                            	<tr>
                            		<td colspan="3">
				                        <a href="<c:url value='/aftercare/insert/form'>
				                        	<c:param name='pet_id' value='${pet_id}'/>
				                        	<c:param name='owner_id' value='${owner_id}'/>
				                        	<c:param name='app_id' value='${app_id}'/>
									      </c:url>"  class="btn btn-primary">게시글 작성</a>
									    <a href="<c:url value='/matching/mymatching'>
									    	</c:url>"  class="btn btn-primary">나가기</a>
			                		</td>
               			 		</tr>
                            </tbody>
                        </table>
